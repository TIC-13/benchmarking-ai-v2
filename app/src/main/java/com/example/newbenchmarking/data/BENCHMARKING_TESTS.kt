...
val BENCHMARKING_TESTS: List<InferenceParams> = MODELS.map {
    if(it.label.startsWith("Yolo"))
        ...
        listOf(
            InferenceParams(it, useNNAPI = false, useGPU = false, numThreads = 1, numImages = 10, DATASETS[0]),
            InferenceParams(it, useNNAPI = true, useGPU = false, numThreads = 1, numImages = 10, DATASETS[0]),
            InferenceParams(it, useNNAPI = false, useGPU = true, numThreads = 1, numImages = 10, DATASETS[0]),
        )
    else
        listOf(
            InferenceParams(it, useNNAPI = false, useGPU = false, numThreads = 1, numImages = 40, DATASETS[0]),
